workspace extends ../eosc-landscape.dsl {
    ...
    model {
        ...
        ai4eosc = group "AI4EOSC" {
            ...
            mlops = softwareSystem "MLOps System" "Manages the lifecycle of machine learning models including deployment, monitoring, and management." {
                ...
                drift_monitoring_system = group "Monitoring" {
                    ...
                    drift_watch -> cd "Triggers actions upon detecting drift"
        ...
        # mlops -> deepaas "Deploy and manage model in production"
        ...
        eosc_user -> model_repo "Registers model implementation code"
        ...
        training_api -> data_repo "retrieves data from repo"
        training_api -> model_repo "trains the model"
        training_api -> ModelRegistry "stores the model version in the registry"
        ModelRegistry -> ci "detects new model version"
        ci -> cd "triggers action to deploy the model"
        cd -> OSCAR "deploy the newly trained model in production"
        mlflow -> drift_watch "tracks the model performance of the deployed model"
        drift_watch -> cd "triggers the pipeline for potential retraining"
    ...
    views {
        ...
        dynamic mlops best_practice_receipt_dynamic {
            title "[Dynamic view] MLOps dynamic view"
            eosc_user -> mlflow "creates a new experiment-run"
            training_api -> mlflow "logs parametres, metrics, models to mlflow"
            training_api -> data_repo "retrieves data from repo"
            training_api -> model_repo "trains the model"
            training_api -> mlflow "stores the model version in the registry"
            mlflow -> ci "detects new model version"
            ci -> cd "triggers action to deploy the model"
            cd -> OSCAR "deploy the new trained model in production"
            mlflow -> frouros "tracks the model performance of the deployed model"
            frouros ->  drift_watch "analyses the performance metrics"
            drift_watch -> cd "triggers the pipeline for potential retraining"
            dashboard -> mlflow "retrieves and displays experiment results and model performance"
            dashboard -> drift_watch "retrieves and displays drift notifications"
        }
        ...
        dynamic mlops data_drift_occurence_view {
            title "[Dynamic view] Data Drift occurence dynamic view"
          #  OSCAR -> frouros "detects new data distribution"
           # frouros -> drift_watch "logs new data drifts metadata"
           # drift_watch -> CD "triggers new actions"
           # drift_watch -> dashboard "informs about the data drift"
            drift_watch -> end_user "notifies the end_user about the data drift"
           # drift_watch -> Training API "if data is valid then trigger model retraining"
            training_api -> data_repo "stores the new data"
            training_api -> model_repo "updates the model with new data"
            ci -> mlflow "picks up the changes in the Model registry and runs tests."
            ci -> cd " If tests pass, CI triggers CD to deploy the updated model

        }

        dynamic mlops model_drift_occurence_view {
            title "[Dynamic view] Model/Concept Drift occurence dynamic view"
          #  OSCAR -> frouros "detects new data distribution"
           # frouros -> drift_watch "logs new data drifts metadata"
           # drift_watch -> CD "triggers new actions"
           # drift_watch -> dashboard "informs about the data drift"
            drift_watch -> eosc_user "notifies the end_user about the data drift"
            # drift_watch -> Training API "triggers model retraining"
            training_api -> data_repo "stores the new data"
            training_api -> model_repo "updates the model with new data"
            ci -> mlflow "picks up the changes in the Model registry and runs tests."
            ci -> cd " If tests pass, CI triggers CD to deploy the updated model
