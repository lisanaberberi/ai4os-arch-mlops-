workspace extends ../eosc-landscape.dsl {
    ...
    views {
        ...
        dynamic mlops best_practice_receipt_dynamic {
            ...
            eosc_user -> mlflow "creates a new experiment-run"
            training_app -> mlflow "logs parametres, metrics, models to mlflow"
            #training_app -> data_repo "retrieves data from repo"
            training_app -> model_repo "trains the model"
            training_app -> mlflow "stores the model version in the registry"
            mlflow -> cicd "triggers action to deploy the model; detects new model version"
        ...
        # when new online data is used during trainig from an external user;
        # after data is valid check a drift occurs
        dynamic mlops data_drift_occurence_view {
          ...
          #  OSCAR -> frouros "detects new data distribution"
           ...
           # drift_watch -> cicd "triggers new actions"
           # drift_watch -> dashboard "informs about the data drift"
            drift_watch -> end_user "notifies the end_user about the data drift"
           # drift_watch -> training_app "if data is valid then trigger model retraining"
           # training_app -> data_repo "stores the new data"
            training_app -> model_repo "updates the model with new data"
            cicd -> mlflow "picks up the changes in the Model registry and runs tests; CI triggers CD to deploy the updated model"
        ...
        dynamic mlops model_drift_occurence_view {
          ...
          #  OSCAR -> frouros "detects new data distribution"
           ...
           # drift_watch -> dashboard "informs about the data drift"
            ...
            cicd -> mlflow "picks up the changes in the Model registry and runs tests."
